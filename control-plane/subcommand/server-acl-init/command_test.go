...
func TestRun_PoliciesAndBindingRulesForACLLogin(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			PolicyNames: []string{"ingress-ingress-gateway-policy",
				"gateway-ingress-gateway-policy",
				"another-gateway-ingress-gateway-policy"},
			Roles: []string{"ingress-ingress-gateway-acl-role",
				"gateway-ingress-gateway-acl-role",
				"another-gateway-ingress-gateway-acl-role"},
		},
	}
	for _, c := range cases {
		t.Run(c.TestName, func(t *testing.T) {
