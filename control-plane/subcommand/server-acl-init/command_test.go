...
func TestRun_PoliciesAndBindingRulesForACLLogin_PrimaryDatacenter(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			Roles: []string{"release-name-consul-terminating-acl-role",
				"release-name-consul-gateway-acl-role",
				"release-name-consul-another-gateway-acl-role"},
		},
	}
	for _, c := range cases {
		t.Run(c.TestName, func(t *testing.T) {
...
func TestRun_PoliciesAndBindingRulesACLLogin_SecondaryDatacenter(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			Roles: []string{"release-name-consul-terminating-acl-role-" + secondaryDatacenter,
				"release-name-consul-gateway-acl-role-" + secondaryDatacenter,
				"release-name-consul-another-gateway-acl-role-" + secondaryDatacenter},
			GlobalAuthMethod: false,
		},
	}
	for _, c := range cases {
	...
	cases := []struct {
		...
		},
		{
			...
			TokenFlags: []string{"-terminating-gateway-name=terminating",
				"-terminating-gateway-name=gateway",
				"-terminating-gateway-name=another-gateway"},
			Roles: []string{"release-name-consul-terminating-acl-role-dc2",
				"release-name-consul-gateway-acl-role-dc2",
				"release-name-consul-another-gateway-acl-role-dc2"},
			GlobalAuthMethod: false,
			GlobalToken:      false,
		},
	}
